...
let custProm = new customPromise((res, rej) => {
  if (2 == 2) {
...
custProm.then((res) => console.log(res));
...
Promise.myAll = (arrayOfPromises) => {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < arrayOfPromises.length; i++) {
      // take let instead of var for correct answer
      Promise.resolve(arrayOfPromises[i])
        .then((data) => {
          // used Promise.resolve here to handle case if taken constant as promise because .then works for only promise
          result[i] = data;
          counter++;
